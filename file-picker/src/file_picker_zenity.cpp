...
std::vector<std::string> ZenityFilePicker::buildCommandLine()
{
  ...
  if (mode == Mode::SAVE) cmd.emplace_back("--save");
  if (!patterns.empty())
  {
...
std::vector<const char*> ZenityFilePicker::convertToC(
...
bool ZenityFilePicker::show()
{
  ...
  if (stat(EXECUTABLE_PATH.c_str(), &sb))
    throw std::runtime_error(
        "Could not find zenity.\n\nTo be able to pick files, please install "
        "the `zenity` utility.");
  ...
  else
  {
    ...
    */
    ...
    if (status == 0)
    {
      ...
      if (iof != std::string::npos) outputStdOut = outputStdOut.substr(0, iof);
