...
namespace mcpe
{
  ...
  class function<ReturnType(ArgTypes...)>
  {
   ...
   public:
    ...
    template <typename Functor,
              typename = typename std::enable_if<
                  !std::is_same<Functor, function>::value>::type>
    ...
    {
      if (manager == nullptr) throw std::bad_function_call();
   ...
   private:
    ...
    {
      // if the size of the functor is smaller than the size of the AnyValue,
      // it'll be stored directly
    ...
    {
      return isFunctorStoredLocally<Functor>() ? (Functor*)(void*)&src
                                               : (Functor*)src.object;
    }
    ...
    static void initFunctor(AnyValue& functor, Functor&& val)
    {
      if (!isFunctorStoredLocally<Functor>())
        ...
        new (getFunctorPointer<Functor>(functor))
            Functor(std::forward<Functor>(val));
    }

    template <typename Functor>
    static ReturnType invokerFunc(const AnyValue& functor, ArgTypes... args)
    {
      return (*getFunctorPointer<Functor>(functor))(
          std::forward<ArgTypes>(args)...);
    }

    template <typename Functor>
    static bool managerFunc(AnyValue& dest, const AnyValue& src,
                            ManagerOperation op)
    {
      switch (op)
      {
        ...
        case ManagerOperation::CloneFunctor:
          if (!isFunctorStoredLocally<Functor>())
            ...
            new (getFunctorPointer<Functor>(dest))
                Functor(*getFunctorPointer<Functor>(src));
          break;
