  ...
  struct string
  {
   private:
    struct _Rep
    {
      ...
      void removeRef()
      {
        if (refcount.fetch_sub(1) == 0) delete this;
      }
    };

    void* ptr;

    static _Rep* createRep(const char* data, size_t length,
                           size_t capacity = 0);
    ...
    {
      if (rep != empty->getRep()) rep->addRef();
      if (ptr != empty->ptr) getRep()->removeRef();
      ptr = (void*)(rep + 1);
    }
