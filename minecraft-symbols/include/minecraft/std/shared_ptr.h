...
namespace mcpe
{
     ...
     protected:
      struct RefCountBase
      {
        ...
        void release() noexcept
        {
          ...
          {
            dispose();
            if (weakCount.fetch_add(-1) == 1) destroy();
          }
        }
      };
      ...
      {
        using RebindedAlloc =
            typename Alloc::template rebind<DeleterRefCount>::other;

        Deleter deleter;
        Alloc alloc;
        T t;

        DeleterRefCount(Deleter deleter, Alloc alloc, T t)
            : deleter(deleter), alloc(alloc), t(t)
        {
        }
  ...
  class shared_ptr : public detail::SharedPtrBase
  {
   ...
   public:
    ...
    ~shared_ptr()
    {
      if (refCount != nullptr) refCount->release();
    }
    ...
    shared_ptr(P ptr, Deleter deleter, Alloc alloc) : ptr(ptr)
    {
      ...
      refCount =
          std::allocator_traits<RefCount>::allocate(ra, sizeof(RefCount));
