...
VersionData *find_version_data(VersionData *versions, int nversions,
                               void *handle)
{
  soinfo *si = (soinfo *)handle;
  for (int i = 0; i < nversions; i++)
  {
    if (versions[i].version_code_addr > si->size) continue;
    if (*(unsigned int *)(si->base + versions[i].version_code_addr) ==
...
void set_current_version_data(VersionData *vd)
{
  ...
  if (!vd) return;
  for (auto vt = vd->vtables; vt->what; vt++) *vt->what = vt->value;
}
  ...
  if (current_version_data != nullptr)
  {
    ...
    auto b =
        current_version_data
            ->symbol_buckets[hash % current_version_data->symbol_bucket_count];
    for (; b->name; b++)
...
void minecraft_get_version(int *major, int *minor, int *patch, int *revision)
{
  ...
  if (SharedConstants::MajorVersion && SharedConstants::MinorVersion &&
