  ...
  class arg_parser
  {
   ...
   public:
    ...
    bool parse(int argc, const char** argv)
    {
      ...
      if (list.next_or_null() == nullptr) return false;
      const char* key;
    ...
    void add_arg(std::string const& name, std::string const& shortname,
                 std::string const& desc, handler h)
    {
    ...
    void print_help()
    {
      ...
      for (auto const& e : help_entries)
        printf("%-2s %-15s %s\n", e.shortname.c_str(), e.name.c_str(),
               e.desc.c_str());
    }
  };
