...
namespace argparser
{
  ...
  class arg
  {
   ...
   public:
    arg(arg_parser& parser, std::string const& name,
        std::string const& shortname, std::string const& desc, T def = T())
        : value(def)
    {
      parser.add_arg(
          name, shortname, desc,
          std::bind(&arg<T>::handle_value, this, std::placeholders::_1));
    }
  ...
  void arg<bool>::handle_value(arg_list& list)
  {
    ...
    if (v == nullptr || strcmp(v, "1") == 0 || strcasecmp(v, "true") == 0 ||
        strcasecmp(v, "on") == 0 || strcasecmp(v, "yes") == 0)
      value = true;
    else if (strcmp(v, "0") == 0 || strcasecmp(v, "false") == 0 ||
