...
simpleipc::client::rpc_call<std::vector<BaseAccountInfo>>
ServiceClient::getAccounts()
{
  return simpleipc::client::rpc_call<std::vector<BaseAccountInfo>>(
      rpc("msa/get_accounts", nullptr),
...
simpleipc::client::rpc_call<void> ServiceClient::addAccount(
    std::string const& cid, std::string const& puid,
...
simpleipc::client::rpc_call<void> ServiceClient::removeAccount(
...
simpleipc::client::rpc_call<std::string> ServiceClient::pickAccount(
    std::string const& clientId, const std::string& cobrandId)
{
  nlohmann::json data;
  if (!clientId.empty()) data["client_id"] = clientId;
  if (!cobrandId.empty()) data["cobrandid"] = cobrandId;
  return simpleipc::client::rpc_call<std::string>(
      rpc("msa/pick_account", data),
      [](nlohmann::json const& d)
      {
...
simpleipc::client::rpc_call<std::shared_ptr<Token>> ServiceClient::requestToken(
    ...
    std::string const& clientId, bool silent)
{
  nlohmann::json data;
  data["cid"] = cid;
