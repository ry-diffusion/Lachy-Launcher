...
extern struct _eglut_xinput_rt_t
{
  ...
  Status (*SelectEvents)(Display* dpy, Window win, XIEventMask* masks,
                         int num_masks);
  Status (*GrabDevice)(Display* dpy, int devid, Window win, Time time,
                       Cursor cursor, int mode, int paired_mode, Bool owner_ev,
                       XIEventMask* mask);
