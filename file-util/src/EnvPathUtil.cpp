...
bool EnvPathUtil::findInPath(std::string const& what, std::string& result,
                             const char* path, const char* cwd)
{
  if (path == nullptr)
  {
    path = getenv("PATH");
    if (path == nullptr || strlen(path) == 0) path = "/bin:/usr/bin";
  }
  ...
  while (path != nullptr)
  {
    ...
    if (pathn == nullptr) len = strlen(path);
    bool willPrefixWithCwd = len == 0 || path[0] != '/';
    bool willReplaceWithDot = len == 0 && cwdLen == 0;
    bool willAppendSlash =
        (willReplaceWithDot ? true : (len != 0 && path[len - 1] != '/'));
    buf.resize((willPrefixWithCwd ? cwdWithSlashLen : 0) +
               (willReplaceWithDot ? 1 : len) + (willAppendSlash ? 1 : 0) +
               what.size());
    size_t o = 0;
