...
void* ModLoader::loadMod(std::string const& path)
{
  ...
  if (handle == nullptr)
  {
    Log::error("ModLoader", "Failed to load mod %s: %s", path.c_str(),
               hybris_dlerror());
    return nullptr;
  }
  ...
  {
    Log::warn("ModLoader", "Mod %s does not have an init function",
              path.c_str());
    return handle;
  }
  initFunc();
...
void ModLoader::loadModMulti(std::string const& path,
...
void ModLoader::onGameInitialized(MinecraftGame* game)
{
  ...
  for (void* mod : mods)
  {
    void (*initFunc)(MinecraftGame*) =
        (void (*)(MinecraftGame*))hybris_dlsym(mod, "mod_set_minecraft");
    if ((void*)initFunc != nullptr) initFunc(game);
  }
}
...
void ModLoader::onServerInstanceInitialized(ServerInstance* server)
{
  ...
  for (void* mod : mods)
  {
    void (*initFunc)(ServerInstance*) =
        (void (*)(ServerInstance*))hybris_dlsym(mod, "mod_set_server");
    if ((void*)initFunc != nullptr) initFunc(server);
  }
}
