...
HookManager::LibInfo::LibInfo(void *handle) : handle(handle)
{
  ...
  for (int i = 0;; i++)
  {
    if (dynData[i].d_tag == DT_NULL) break;
  ...
  {
    Elf32_Phdr &entry = *((Elf32_Phdr *)((size_t)base + header->e_phoff +
...
void HookManager::LibInfo::applyHooks(Elf32_Rel *rel, Elf32_Word relsz)
{
  for (size_t i = 0; i < relsz / sizeof(Elf32_Rel); i++)
  {
    ...
    if (found_symbol == hookedSymbols.end()) continue;
    ...
    if (symInfo.lastHook != nullptr && symInfo.lastHook->replacement != nullptr)
      ...
      continue;
    Log::trace(TAG, "Found hook for %s at %x", &strtab[symtab[sym].st_name],
               addr);
    ...
    if (original && symInfo.original == nullptr)
    {
      ...
      if (symInfo.firstHook) *symInfo.firstHook->orig = (void *)original;
    }
  }
}
...
void HookManager::addLibrary(void *handle)
{
  ...
  for (int i = 0;; i++)
  {
    ...
    {
      void *dep =
          hybris_dlopen((const char *)dynData[i].d_un.d_val, RTLD_NOLOAD);
...
void HookManager::removeLibrary(void *handle)
{
  ...
  if (p == libs.end()) return;
  for (auto const &dep : p->second->dependencies)
    dependents[dep].erase(std::remove(dependents[dep].begin(),
                                      dependents[dep].end(), p->second.get()),
                          dependents[dep].end());
  libs.erase(p);
}
    ...
    void *lib, Elf32_Word symbolIndex)
{
  ...
  if (lib_ir == libs.end()) throw std::runtime_error("No such lib registered");
  LibInfo *lib_i = lib_ir->second.get();
  auto &hook = hookedSymbols[{ lib_i, symbolIndex }];
  if (hook != nullptr) return hook.get();
  ...
  {
    for (LibInfo *dep : deps->second) dep->setHook(name, hook);
  }
  return hook.get();
}

HookManager::HookInstance *HookManager::createHook(void *lib,
                                                   ...
                                                   void **orig)
{
  ...
  if (sym_index == (Elf32_Word)-1) throw std::runtime_error("No such symbol");
...
void HookManager::deleteHook(HookInstance *hook)
{
  ...
  if (hook->parent) hook->parent->child = hook->child;
  if (hook->symbol->firstHook == hook) hook->symbol->firstHook = hook->child;
  if (hook->symbol->lastHook == hook) hook->symbol->lastHook = hook->parent;
  delete hook;
}

void HookManager::applyHooks()
{
  for (auto const &lib : libs) lib.second->applyHooks();
}
...
Elf32_Word HookManager::getSymbolIndex(void *lib, const char *symbolName)
{
  ...
  for (Elf32_Word index = slib->bucket[hash]; index != 0;
       index = slib->chain[index])
  {
...
std::string HookManager::translateConstructorName(const char *name)
{
  ...
  if (sl < 2 || memcmp(name, "_Z", 2) != 0) return std::string();
  s += 2;
  while (s < se)
  {
    if (*s == 'N')
