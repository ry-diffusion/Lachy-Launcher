...
class LauncherAppPlatform : public AppPlatform
{
 ...
 public:
  ...
  mcpe::string region;
  mcpe::string internalStorage, externalStorage, currentStorage, userdata,
      userdataPathForLevels, tmpPath;
  ...
  mcpe::string getDataUrl()
  {  // this is used only for sounds
  ...
  {
    // Log::trace(TAG, "getUserdataPathForLevels: %s",
    // userdataPathForLevels.c_str());
  ...
  mcpe::string getAssetFileFullPath_pre_1_13(
      Legacy::Pre_1_13::Core::Path const& s)
  {
    // Log::trace(TAG, "getAssetFileFullPath: %s", s.c_str());
    std::string ret = assetsDir;
    if (s.size != (size_t)-1)
  ...
  {
    if (enablePocketGuis) return "pocket";
    return "win10";
  }
  ...
  void runMainThreadTasks()
  {
    ...
    runOnMainThreadMutex.unlock();
    for (auto const& func : queue) func();
  }
  ...
  void loadPNG_pre_0_14(Legacy::Pre_0_14::ImageData& imgData,
