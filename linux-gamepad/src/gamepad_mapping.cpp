...
{
  auto p = s.find(c, a);
  if (p == std::string::npos) throw std::invalid_argument("Invalid mapping");
  return p;
}
...
std::unordered_map<std::string, GamepadButton>
GamepadMapping::createKnownButtonList()
{
...
std::unordered_map<std::string, GamepadAxis>
GamepadMapping::createKnownAxisList()
{
...
{
  static std::unordered_map<std::string, GamepadButton> knownButtons =
      createKnownButtonList();
  static std::unordered_map<std::string, GamepadAxis> knownAxis =
      createKnownAxisList();
  ...
  while (iof != std::string::npos && iof + 1 < mapping.length())
  {
    ...
    // parse target
    char toMod = 0;
    if (mapping[iof] == '-' || mapping[iof] == '+') toMod = mapping[iof++];
    ...
    else if (axis != knownAxis.end())
    {
      ...
      if (axis->second == GamepadAxis::LEFT_TRIGGER ||
          axis->second == GamepadAxis::RIGHT_TRIGGER)
      {
        c.to.d.axis.min = 0.f;
        c.to.d.axis.max = 1.f;
      }
    ...
    // parse source
    char fromMod = 0;
    if (mapping[iof] == '-' || mapping[iof] == '+') fromMod = mapping[iof++];
    bool inv = false;
    if (mapping[iof] == '~')
    {
      inv = true;
    ...
    else if (mapping[iof] == 'a')
    {
      ...
      if (inv) std::swap(c.to.d.axis.min, c.to.d.axis.max);
    }
