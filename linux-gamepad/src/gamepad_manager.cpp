...
GamepadManager::GamepadManager(JoystickManager& jsManager)
    : jsManager(jsManager)
{
  using namespace std::placeholders;
  jsManager.onJoystickConnected.add(
      *this, std::bind(&GamepadManager::onJoystickConnected, this, _1));
  jsManager.onJoystickDisconnected.add(
      *this, std::bind(&GamepadManager::onJoystickDisconnected, this, _1));
  jsManager.onJoystickButton.add(
      *this, std::bind(&GamepadManager::onJoystickButton, this, _1, _2, _3));
  jsManager.onJoystickAxis.add(
      *this, std::bind(&GamepadManager::onJoystickAxis, this, _1, _2, _3));
  jsManager.onJoystickHat.add(
      *this, std::bind(&GamepadManager::onJoystickHat, this, _1, _2, _3));
}
...
int GamepadManager::takeGamepadId()
{
  ...
  if (p + 1 == takenGamepadLowId) takenGamepadLowId++;
  return p + 1;
}

void GamepadManager::putGamepadIdBack(int i)
{
  if (i < takenGamepadLowId) takenGamepadLowId = i;
...
void GamepadManager::onJoystickButton(Joystick* js, int button, bool state)
{
  ...
  for (auto const& map : gp->getMapping().mappings)
  {
    ...
    {
      onGamepadAxis(gp, map.to.d.axis.id,
                    state ? map.to.d.axis.max : map.to.d.axis.min);
    }
  }
}
...
void GamepadManager::onJoystickAxis(Joystick* js, int axis, float value)
{
  ...
  for (auto const& map : gp->getMapping().mappings)
  {
    if (map.from.type != GamepadMapping::MapFrom::Type::AXIS ||
        map.from.d.axis.id != axis)
      continue;
    if (map.to.type == GamepadMapping::MapTo::Type::BUTTON)
    {
      onGamepadButton(gp, map.to.d.button.id,
                      GamepadMapping::isAxisActive(map.from, value));
    }
    else if (map.to.type == GamepadMapping::MapTo::Type::AXIS)
    {
...
void GamepadManager::onJoystickHat(Joystick* js, int hat, int value)
{
  ...
  for (auto const& map : gp->getMapping().mappings)
  {
    if (map.from.type != GamepadMapping::MapFrom::Type::HAT ||
        map.from.d.hat.id != hat)
      continue;
