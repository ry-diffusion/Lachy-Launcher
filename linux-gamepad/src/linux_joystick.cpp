    ...
    : mgr(mgr), devPath(path), edev(edev)
{
  ...
  memset(buttons, 0xff,
         sizeof(buttons));  // sets all the buttons to -1 by default
  ...
  if (len < 0) throw std::runtime_error("Failed to get joystick keys");
  len *= 8;
  ...
  {
    if (TEST_BIT(keys, i % len)) buttons[i] = nextId++;
  }

  unsigned long abs[NLONGS(ABS_CNT)];
  len = ioctl(fd, EVIOCGBIT(EV_ABS, sizeof(abs)), abs);
  if (len < 0) throw std::runtime_error("Failed to get joystick abs");
  len *= 8;
  ...
  for (size_t i = 0; i < len; i++)
  {
    ...
    axis[i] = { id, absinfo->minimum, absinfo->maximum, absinfo->flat,
                absinfo->fuzz };
  }
  nextId = 0;
  for (size_t i = ABS_HAT0X; i <= ABS_HAT3Y; i += 2)
  {
    if (axis[i].index == -1 && axis[i + 1].index == -1) continue;
...
void LinuxJoystick::poll()
{
  ...
  while (true)
  {
    ...
    if (e.type == EV_KEY)
    {
      if (e.code >= KEY_CNT) continue;
      int btn = buttons[e.code];
      if (btn == -1) continue;
      bool v = e.value != 0;
      buttons[btn] = v;
      if (mgr) mgr->onJoystickButton(this, btn, v);
    }
    else if (e.type == EV_ABS && isHat(e.code))
    {
      ...
      hatValues[a.index] = v;
      if (mgr) mgr->onJoystickHat(this, a.index, v);
    }
    else if (e.type == EV_ABS)
    {
      if (e.code >= AXIS_COUNT) continue;
      auto& a = axis[e.code];
      if (a.index == -1) continue;
      ...
      if (std::abs(iv) < a.flat) v = 0.f;
      v = std::min(std::max(v, -1.f), 1.f);
      axisValues[a.index] = v;
      if (mgr) mgr->onJoystickAxis(this, a.index, v);
    }
  }
}
