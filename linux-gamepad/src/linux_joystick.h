...
namespace gamepad
{
  ...
  class LinuxJoystick : public Joystick
  {
   ...
   public:
    LinuxJoystick(LinuxJoystickManager* mgr, std::string const& path,
                  struct libevdev* edev);
    ...
    {
      if (index < 0 || index >= BUTTON_COUNT) return false;
      return buttonValues[index];
    }

    float getAxis(int index) const override
    {
      if (index < 0 || index >= AXIS_COUNT) return false;
      return axisValues[index];
    }

    int getHat(int index) const override
    {
      if (index < 0 || index >= HAT_COUNT) return false;
