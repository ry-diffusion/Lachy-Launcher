...
namespace gamepad
{
  ...
  class CallbackAutoHandler
  {
   ...
   public:
    ...
    void addCallbackDeleter(void* list, CallbackId id,
                            void (*deleter)(void* list, CallbackId id))
    {
  ...
  class CallbackList
  {
   ...
   public:
    ...
    {
      h.addCallbackDeleter(this, add(t),
                           [](void* list, CallbackId id)
                           {
    ...
    {
      for (auto const& c : callbacks) c.value(args...);
    }
  };

  inline CallbackAutoHandler::~CallbackAutoHandler()
  {
    for (auto& p : callbacks) p.deleter(p.list, p.id);
  }
