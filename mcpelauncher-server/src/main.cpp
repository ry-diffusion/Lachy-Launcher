...
int main(int argc, char* argv[])
{
  ...
  Whitelist whitelist;
  PermissionsFile permissionsFile(PathHelper::getPrimaryDataDirectory() +
                                  "permissions.json");
  ...
  ResourceLoaders::registerLoader(
      ...
      std::unique_ptr<ResourceLoader>(new AppResourceLoader(
          [&pathmgr]
          {
            return pathmgr.getPackagePath();
          })));
  // ResourceLoaders::registerLoader((ResourceFileSystem) 7,
  // std::unique_ptr<ResourceLoader>(new AppResourceLoader([&pathmgr] { return
  // pathmgr.getDataUrl(); })));
  ResourceLoaders::registerLoader(
      ...
      std::unique_ptr<ResourceLoader>(new AppResourceLoader(
          [&pathmgr]
          {
            return pathmgr.getUserDataPath();
          })));
  ResourceLoaders::registerLoader(
      ...
      std::unique_ptr<ResourceLoader>(new AppResourceLoader(
          [&pathmgr]
          {
            return pathmgr.getSettingsPath();
          })));
  // ResourceLoaders::registerLoader((ResourceFileSystem) 5,
  // std::unique_ptr<ResourceLoader>(new AppResourceLoader([&pathmgr] { return
  // pathmgr.getExternalFilePath(); })));
  // ResourceLoaders::registerLoader((ResourceFileSystem) 2,
  // std::unique_ptr<ResourceLoader>(new AppResourceLoader([&pathmgr] { return
  // ""; }))); ResourceLoaders::registerLoader((ResourceFileSystem) 3,
  // std::unique_ptr<ResourceLoader>(new AppResourceLoader([&pathmgr] { return
  // ""; }))); ResourceLoaders::registerLoader((ResourceFileSystem) 9,
  // std::unique_ptr<ResourceLoader>(new ScreenshotLoader));
  // ResourceLoaders::registerLoader((ResourceFileSystem) 0xA,
  // std::unique_ptr<ResourceLoader>(new AppResourceLoader([&pathmgr] { return
  // ""; })));
  ...
  auto createLevelStorageFunc =
      [&levelStorage, &props, keyProvider](Scheduler& scheduler)
  {
    return levelStorage.createLevelStorage(
        scheduler, props.worldDir.get(), *ContentIdentity::EMPTY, *keyProvider);
  };
  std::unique_ptr<EducationOptions> eduOptions(
      new EducationOptions(resourcePackManager));
  ServerInstanceEventCoordinator instanceEventCoordinator;
  std::unique_ptr<ServerInstance> instance(
      new ServerInstance(minecraftApp, instanceEventCoordinator));
  ...
  instance->initializeServer(
      minecraftApp, whitelist, &permissionsFile, &pathmgr, idleTimeout,
      props.worldDir.get(), props.worldName.get(), props.motd.get(),
      levelSettings, props.viewDistance, true,
      { props.port, props.portV6, props.maxPlayers }, props.onlineMode, {},
      "normal", *mce::UUID::EMPTY, eventing, resourcePackRepo, ctm,
      *resourcePackManager, createLevelStorageFunc, worldspath, nullptr,
      mcpe::string(), mcpe::string(), std::move(eduOptions),
      resourcePackManager,
      []()
      {
  ...
  while (reader.read(line))
  {
    instance->queueForServerThread(
        [&instance, line]()
        {
          std::unique_ptr<ServerCommandOrigin> commandOrigin(
              new ServerCommandOrigin(
                  "Server", (ServerLevel&)*instance->minecraft->getLevel(),
                  CommandPermissionLevel::ServerConsole));
