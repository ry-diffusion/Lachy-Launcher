...
/* The following pcm representations and data formats map to those in
 * OpenSLES 1.1 */
...
#define SL_ANDROID_SPEAKER_USE_DEFAULT ((SLuint32)0)
  ...
  struct SLAndroidEffectItf_
  {
    ...
    SLresult (*SendCommand)(SLAndroidEffectItf self,
                            SLInterfaceID effectImplementationId,
                            SLuint32 command, SLuint32 commandSize,
                            void *pCommandData, SLuint32 *replySize,
                            void *pReplyData);
  };
  ...
  struct SLAndroidEffectSendItf_
  {
    SLresult (*EnableEffectSend)(SLAndroidEffectSendItf self,
                                 SLInterfaceID effectImplementationId,
                                 SLboolean enable, SLmillibel initialLevel);
    SLresult (*IsEnabled)(SLAndroidEffectSendItf self,
                          SLInterfaceID effectImplementationId,
                          SLboolean *pEnable);
    SLresult (*SetDirectLevel)(SLAndroidEffectSendItf self,
                               SLmillibel directLevel);
    SLresult (*GetDirectLevel)(SLAndroidEffectSendItf self,
                               SLmillibel *pDirectLevel);
    SLresult (*SetSendLevel)(SLAndroidEffectSendItf self,
                             SLInterfaceID effectImplementationId,
                             SLmillibel sendLevel);
    SLresult (*GetSendLevel)(SLAndroidEffectSendItf self,
                             SLInterfaceID effectImplementationId,
  ...
  typedef const struct SLAndroidEffectCapabilitiesItf_ *const
  ...
  struct SLAndroidEffectCapabilitiesItf_
  {
    ...
    SLresult (*QueryEffect)(SLAndroidEffectCapabilitiesItf self, SLuint32 index,
                            SLInterfaceID *pEffectType,
                            SLInterfaceID *pEffectImplementation, SLchar *pName,
                            SLuint16 *pNameSize);
  };
...
#define SL_ANDROID_JAVA_PROXY_ROUTING 0x0001
  ...
  struct SLAndroidConfigurationItf_
  {
    ...
    SLresult (*GetConfiguration)(SLAndroidConfigurationItf self,
                                 const SLchar *configKey, SLuint32 *pValueSize,
                                 void *pConfigValue);

    SLresult (*AcquireJavaProxy)(SLAndroidConfigurationItf self,
                                 SLuint32 proxyType, jobject *pProxyObj);
  ...
  struct SLAndroidSimpleBufferQueueItf_
  {
    SLresult (*Enqueue)(SLAndroidSimpleBufferQueueItf self, const void *pBuffer,
                        SLuint32 size);
...
#define SL_ANDROIDBUFFERQUEUEEVENT_CONTENT_END ((SLuint32)0x00000020)
  ...
  struct SLAndroidBufferQueueItf_
  {
    SLresult (*RegisterCallback)(SLAndroidBufferQueueItf self,
                                 slAndroidBufferQueueCallback callback,
                                 void *pCallbackContext);

    SLresult (*Clear)(SLAndroidBufferQueueItf self);

    SLresult (*Enqueue)(SLAndroidBufferQueueItf self, void *pBufferContext,
                        void *pData, SLuint32 dataLength,
                        const SLAndroidBufferItem *pItems,
                        SLuint32 itemsLength);

    SLresult (*GetState)(SLAndroidBufferQueueItf self,
                         SLAndroidBufferQueueState *pState);

    SLresult (*SetCallbackEventsMask)(SLAndroidBufferQueueItf self,
                                      SLuint32 eventFlags);

    SLresult (*GetCallbackEventsMask)(SLAndroidBufferQueueItf self,
...
#define SL_DATALOCATOR_ANDROIDFD_USE_FILE_SIZE ((SLAint64)0xFFFFFFFFFFFFFFFFll)
  ...
  /** File Descriptor-based data locator definition, locatorType must be
...
#define SL_ANDROID_MIME_AACADTS ((SLchar *)"audio/vnd.android.aac-adts")
  ...
  struct SLAndroidAcousticEchoCancellationItf_
  {
    SLresult (*SetEnabled)(SLAndroidAcousticEchoCancellationItf self,
                           SLboolean enabled);
    SLresult (*IsEnabled)(SLAndroidAcousticEchoCancellationItf self,
                          SLboolean *pEnabled);
  };
  ...
  typedef const struct SLAndroidAutomaticGainControlItf_ *const
      *SLAndroidAutomaticGainControlItf;

  struct SLAndroidAutomaticGainControlItf_
  {
    SLresult (*SetEnabled)(SLAndroidAutomaticGainControlItf self,
                           SLboolean enabled);
    SLresult (*IsEnabled)(SLAndroidAutomaticGainControlItf self,
                          SLboolean *pEnabled);
  };
  ...
  typedef const struct SLAndroidNoiseSuppressionItf_ *const
      *SLAndroidNoiseSuppressionItf;

  struct SLAndroidNoiseSuppressionItf_
  {
    SLresult (*SetEnabled)(SLAndroidNoiseSuppressionItf self,
                           SLboolean enabled);
    SLresult (*IsEnabled)(SLAndroidNoiseSuppressionItf self,
                          SLboolean *pEnabled);
  };
