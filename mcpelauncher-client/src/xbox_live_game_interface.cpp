...
{
  if (!get_uploader_x_token_callback) return std::string();
  auto val =
      ((jnivm::java::lang::String * (*)(JNIEnv * env, jclass cl, jboolean))
           get_uploader_x_token_callback)(jnienv, nullptr, refresh);
  return val ? *val : std::string();
}

std::string XboxLiveDefaultGameInterface::getCllXTicket(std::string const& xuid)
{
  if (!get_supporting_x_token_callback) return std::string();
  auto val =
      ((jnivm::java::lang::String * (*)(JNIEnv * env, jclass cl, jstring))
           get_supporting_x_token_callback)(jnienv, nullptr,
                                            jnienv->NewStringUTF(xuid.data()));
  return val ? *val : std::string();
}