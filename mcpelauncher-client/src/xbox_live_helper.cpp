...
std::string XboxLiveHelper::findMsa()
{
  ...
  if (EnvPathUtil::findInPath("msa-daemon", path, MSA_DAEMON_PATH,
    ...
    return path;
#endif
  if (EnvPathUtil::findInPath("msa-daemon", path)) return path;
  return std::string();
}
...
msa::client::ServiceClient* XboxLiveHelper::getMsaClientOrNull()
{
  ...
  if (client.get()) return client.get();
  try
  {
...
msa::client::ServiceClient& XboxLiveHelper::getMsaClient()
{
  ...
  if (!client) throw std::runtime_error("Could not connect to the daemon");
  return *client;
}
...
void XboxLiveHelper::initCll(std::string const& cid)
{
  ...
  if (!cid.empty()) cllAuthStep.setAccount(cid);
  if (cll) return;
  auto tid = xbox::services::xbox_live_app_config::get_app_config_singleton()
                 ->title_id();
