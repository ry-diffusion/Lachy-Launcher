...
#if defined(DEBUG)
#define HYBRIS_LOG_(level, module, message, ...)                          \
  do                                                                      \
  {                                                                       \
    ...
    {                                                                     \
      ...
      {                                                                   \
        fprintf(hybris_logging_target,                                    \
                "B|%i|%.9f|%s(%s) %s:%d (%s) " message "\n", getpid(),    \
                hybris_get_thread_time(), module, __PRETTY_FUNCTION__,    \
                __FILE__, __LINE__,                                       \
                #level + 11 /* + 11 = strip leading "HYBRIS_LOG_" */,     \
                ##__VA_ARGS__);                                           \
        fflush(hybris_logging_target);                                    \
        fprintf(hybris_logging_target,                                    \
                "E|%i|%.9f|%s(%s) %s:%d (%s) " message "\n", getpid(),    \
                hybris_get_thread_time(), module, __PRETTY_FUNCTION__,    \
                __FILE__, __LINE__,                                       \
                #level + 11 /* + 11 = strip leading "HYBRIS_LOG_" */,     \
                ##__VA_ARGS__);                                           \
...
#define HYBRIS_TRACE_RECORD(module, what, tracepoint, message, ...)           \
  do                                                                          \
  {                                                                           \
    ...
    {                                                                         \
      ...
      {                                                                       \
        fprintf(hybris_logging_target,                                        \
                "PID: %i TTIME: %.9f Tracepoint-%c/%s::%s" message "\n",      \
      ...
      {                                                                       \
        if (what == 'B')                                                      \
          fprintf(hybris_logging_target, "B|%i|%.9f|%s::%s" message "",       \
                  getpid(), hybris_get_thread_time(), tracepoint, module,     \
                  ##__VA_ARGS__);                                             \
        else if (what == 'E')                                                 \
...
#define HYBRIS_TRACE_BEGIN(module, tracepoint, message, ...) \
  HYBRIS_TRACE_RECORD(module, 'B', tracepoint, message, ##__VA_ARGS__)
#define HYBRIS_TRACE_END(module, tracepoint, message, ...) \
  HYBRIS_TRACE_RECORD(module, 'E', tracepoint, message, ##__VA_ARGS__)
#define HYBRIS_TRACE_COUNTER(module, tracepoint, message, ...) \
  HYBRIS_TRACE_RECORD(module, 'C', tracepoint, message, ##__VA_ARGS__)
#else
#define HYBRIS_LOG_(level, module, message, ...) \
  while (0)                                      \
  {                                              \
...
#define HYBRIS_DEBUG_LOG(module, message, ...) \
  HYBRIS_LOG_(HYBRIS_LOG_DEBUG, #module, message, ##__VA_ARGS__)
#define HYBRIS_WARN_LOG(module, message, ...) \
  HYBRIS_LOG_(HYBRIS_LOG_WARN, #module, message, ##__VA_ARGS__)
#define HYBRIS_INFO_LOG(module, message, ...) \
  HYBRIS_LOG_(HYBRIS_LOG_INFO, #module, message, ##__VA_ARGS__)
#define HYBRIS_ERROR_LOG(module, message, ...) \
  HYBRIS_LOG_(HYBRIS_LOG_ERROR, #module, message, ##__VA_ARGS__)

/* Module-specific logging functions can be defined like this */
#define HYBRIS_DEBUG(message, ...) \
  HYBRIS_DEBUG_LOG(HYBRIS, message, ##__VA_ARGS__)
#define HYBRIS_WARN(message, ...) \
  HYBRIS_WARN_LOG(HYBRIS, message, ##__VA_ARGS__)
#define HYBRIS_INFO(message, ...) \
  HYBRIS_INFO_LOG(HYBRIS, message, ##__VA_ARGS__)
#define HYBRIS_ERROR(message, ...) \
  HYBRIS_ERROR_LOG(HYBRIS, message, ##__VA_ARGS__)
