...
namespace daemon_utils
{
  ...
  class auto_shutdown_service : public simpleipc::server::service
  {
   ...
   public:
    auto_shutdown_service(std::unique_ptr<simpleipc::server::service_impl> impl,
                          shutdown_policy policy)
        : service(std::move(impl)), shutdown_mode(policy)
    {
    }

    auto_shutdown_service(std::string const& path, shutdown_policy policy)
        : service(path), shutdown_mode(policy)
    {
    }
    ...
    {
      if (--connections == 0 &&
          shutdown_mode == shutdown_policy::no_connections)
      {
        request_stop();
      }
    }
