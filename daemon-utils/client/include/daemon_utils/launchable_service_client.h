...
namespace daemon_utils
{
  ...
  class launchable_service_client : public simpleipc::client::service_client
  {
   ...
   public:
    explicit launchable_service_client(std::string const& path)
        : service_client(path)
    {
    }

    explicit launchable_service_client(
        std::unique_ptr<simpleipc::client::service_client_impl> impl)
        : service_client(std::move(impl))
    {
    }

    explicit launchable_service_client(daemon_launcher& launcher)
        : service_client(simpleipc::client::service_client_impl_factory::
   ...
   protected:
    void connection_closed() override
    {
      ...
      if (!shutting_down && launcher) launcher->open(get_impl());
    }
  };
