...
pid_t daemon_launcher::start()
{
  ...
  if (!(ret = fork()))
  {
    ...
    for (int i = 0; i < args.size(); i++) argv[i] = &args[i][0];
  ...
  if (ret < 0) throw std::runtime_error("fork failed");
  return ret;
}
...
void daemon_launcher::open(simpleipc::client::service_client_impl& impl)
{
  ...
  if (S_ISSOCK(s.st_mode))
  {
    ...
    {
      // open failed
      Log::info("DaemonLauncher",
                "Daemon file exists, but we could not open it (%s); "
                "starting the service anyways",
                e.what());
    }
  }
  ...
  while (true)
  {
    long long remaining = std::chrono::duration_cast<std::chrono::milliseconds>(
                              ...
                              .count();
    if (remaining <= 50)  // exit the loop anyways if we're only 50ms from
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      {  // Change happened in the directory
  ...
  simpleipc::io_handler::get_instance().add_socket(
      sfd,
      [proc, &stop_promise](int fd)
      {
        int status;
        if (waitpid(proc, &status, WNOHANG) == -1) return;
  ...
  simpleipc::io_handler::get_instance().add_socket(
      fd,
      [wd, &stop_promise, service_filename](int fd)
      {
        unsigned int av;
