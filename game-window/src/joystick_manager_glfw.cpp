...
std::unordered_map<int, GLFWJoystickManager::JoystickInfo>
...
void GLFWJoystickManager::loadMappingsFromFile(std::string const& path)
{
  ...
  {
    if (!line.empty()) glfwUpdateGamepadMappings(line.c_str());
  }
}
...
void GLFWJoystickManager::update(GLFWGameWindow* window)
{
  ...
  for (auto& j : connectedJoysticks)
  {
    ...
    for (int i = 0; i <= GLFW_GAMEPAD_BUTTON_LAST; i++)
    {
      ...
      {
        window->onGamepadButton(j.second.userId, mapButtonId(i),
                                state.buttons[i] != 0);
      }
    }
    ...
    memcpy(j.second.oldButtonStates, state.buttons,
...
void GLFWJoystickManager::_glfwJoystickCallback(int joystick, int action)
{
  ...
  {
    if (js != connectedJoysticks.end()) return;
