...
EGLUTWindowManager::EGLUTWindowManager()
{
  ...
  eglutInit(0, nullptr);  // the args aren't really required and are troublesome
                          // to pass with this system
}
...
std::shared_ptr<GameWindow> EGLUTWindowManager::createWindow(
    const std::string& title, int width, int height, GraphicsApi api)
{
  return std::shared_ptr<GameWindow>(
      new EGLUTWindow(title, width, height, api));
}
