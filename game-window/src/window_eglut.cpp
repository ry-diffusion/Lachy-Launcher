...
EGLUTWindow::EGLUTWindow(const std::string& title, int width, int height,
                         GraphicsApi api)
    : WindowWithLinuxJoystick(title, width, height, api),
      title(title),
      width(width),
      height(height),
      graphicsApi(api)
...
EGLUTWindow::~EGLUTWindow()
{
  ...
  if (winId != -1) eglutDestroyWindow(winId);
}
...
void EGLUTWindow::setCursorDisabled(bool disabled)
{
  ...
  eglutSetMousePointerLocked(disabled ? EGLUT_POINTER_LOCKED
                                      : EGLUT_POINTER_UNLOCKED);
}

void EGLUTWindow::setFullscreen(bool fullscreen)
{
  if (eglutGet(EGLUT_FULLSCREEN_MODE) !=
      (fullscreen ? EGLUT_FULLSCREEN : EGLUT_WINDOWED))
...
void EGLUTWindow::_eglutReshapeFunc(int w, int h)
{
  if (currentWindow == nullptr ||
      (currentWindow->width == w && currentWindow->height == h))
    return;
...
void EGLUTWindow::_eglutMouseButtonFunc(int x, int y, int btn, int action)
{
  ...
  currentWindow->onMouseButton(x, y, btn,
                               action == EGLUT_MOUSE_PRESS
...
int EGLUTWindow::obtainTouchPointer(int eglutId)
{
  ...
  {
    if (pointerIds[i] == eglutId) return i;
  }
  for (int i = 0; i < sizeof(pointerIds) / sizeof(int); i++)
  {
    if (pointerIds[i] == -1)
...
{
  if (currentWindow == nullptr || strcmp(str, "\t") == 0 ||
      strcmp(str, "\03") == 0 || strcmp(str, "\26") == 0 ||
      strcmp(str, "\33") == 0)  // \t, copy, paste, esc
    return;
  if (action == EGLUT_KEY_PRESS || action == EGLUT_KEY_REPEAT)
  {
    if (str[0] == 13 && str[1] == 0) str[0] = 10;
    std::stringstream ss;
    ss << str;
...
int EGLUTWindow::getKeyMinecraft(int keyCode)
{
  ...
  if (keyCode == 65515)  // tab
    return 8;
  if (keyCode >= 97 && keyCode <= 122) return (keyCode + 65 - 97);
  if (keyCode >= 65361 && keyCode <= 65364) return (keyCode + 37 - 65361);
  if (keyCode >= 65470 && keyCode <= 65481) return (keyCode + 112 - 65470);
...
{
  if (currentWindow == nullptr) return;
  if (key == 65507) currentWindow->modCTRL = (action != EGLUT_KEY_RELEASE);
  if (currentWindow->modCTRL && (key == 86 || key == 118) &&
      action == EGLUT_KEY_PRESS)
  {
    eglutRequestPaste();
  }
  ...
  KeyAction enumAction =
      (action == EGLUT_KEY_PRESS
           ? KeyAction::PRESS
           : (action == EGLUT_KEY_REPEAT ? KeyAction::REPEAT
                                         : KeyAction::RELEASE));
...
{
  if (currentWindow == nullptr) return;
  LinuxGamepadJoystickManager::instance.onWindowFocused(
      currentWindow, (action == EGLUT_FOCUSED));
