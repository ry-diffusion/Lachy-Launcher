...
GLFWGameWindow::GLFWGameWindow(const std::string &title, int width, int height,
                               GraphicsApi api)
    : GameWindow(title, width, height, api), windowedWidth(width),
...
int GLFWGameWindow::getRelativeScale() const { return relativeScale; }

void GLFWGameWindow::getWindowSize(int &width, int &height) const {
  glfwGetFramebufferSize(window, &width, &height);
}

void GLFWGameWindow::show() { glfwShowWindow(window); }

void GLFWGameWindow::close() { glfwSetWindowShouldClose(window, GLFW_TRUE); }

void GLFWGameWindow::prepareRunLoop() { GLFWJoystickManager::addWindow(this); }
...
void GLFWGameWindow::swapInterval(int interval) { glfwSwapInterval(interval); }
...
void GLFWGameWindow::_glfwScrollCallback(GLFWwindow *window, double x,
                                         double y) {
  GLFWGameWindow *user = (GLFWGameWindow *)glfwGetWindowUserPointer(window);
  double cx, cy;
...
int GLFWGameWindow::getKeyMinecraft(int keyCode) {
  ...
  case GLFW_KEY_APOSTROPHE:
    ...
    // Extra key mappings that are not necessarily correct but map to completely
    // wrong keys otherwise
