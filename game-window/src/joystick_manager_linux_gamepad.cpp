...
LinuxGamepadJoystickManager::LinuxGamepadJoystickManager()
    : joystickManager(gamepad::JoystickManagerFactory::create()),
      gamepadManager(*joystickManager)
{
  using namespace std::placeholders;
  gamepadManager.onGamepadConnected.add(
      std::bind(&LinuxGamepadJoystickManager::onGamepadState, this, _1, true));
  gamepadManager.onGamepadDisconnected.add(
      std::bind(&LinuxGamepadJoystickManager::onGamepadState, this, _1, false));
  gamepadManager.onGamepadButton.add(std::bind(
      &LinuxGamepadJoystickManager::onGamepadButton, this, _1, _2, _3));
  gamepadManager.onGamepadAxis.add(
      std::bind(&LinuxGamepadJoystickManager::onGamepadAxis, this, _1, _2, _3));
...
void LinuxGamepadJoystickManager::onWindowFocused(
    WindowWithLinuxJoystick* window, bool focused)
{
  if (focused)
...
void LinuxGamepadJoystickManager::onGamepadState(gamepad::Gamepad* gp,
                                                 bool connected)
{
  if (connected)
    gamepads.insert(gp);
  ...
  for (auto window : windows) window->onGamepadState(gp->getIndex(), connected);
}

void LinuxGamepadJoystickManager::onGamepadButton(gamepad::Gamepad* gp,
                                                  gamepad::GamepadButton btn,
                                                  bool state)
{
...
void LinuxGamepadJoystickManager::onGamepadAxis(gamepad::Gamepad* gp,
...
GamepadButtonId LinuxGamepadJoystickManager::mapButtonId(
    gamepad::GamepadButton id)
{
