...
default_rpc_handler::default_rpc_handler()
{
  ...
  add_handler_async(".hello", std::bind(&default_rpc_handler::handle_hello,
                                        this, _1, _3, _4));
}

void default_rpc_handler::handle_hello(
    connection& conn, nlohmann::json const& data,
    rpc_handler::result_handler const& handler)
{
  ...
  handler(rpc_json_result::response({ { "version", version::current_version },
