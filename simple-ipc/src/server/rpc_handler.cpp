...
void rpc_handler::add_handler_async(std::string const& method,
                                    call_handler_async handler)
{
...
void rpc_handler::invoke(connection& conn, std::string const& method,
                         ...
                         result_handler const& handler)
{
  ...
  if (h == handlers.end())
  {
    handler(rpc_json_result::error(rpc_error_codes::method_not_found,
                                   rpc_error_codes::to_string));
    return;
  }
  try
  {
  ...
  {
    handler(rpc_json_result::error(rpc_error_codes::internal_error,
                                   rpc_error_codes::to_string));
  }
}

void rpc_handler::invoke(std::shared_ptr<connection> conn,
                         rpc_message const& msg)
{
  if (msg.has_id())
  {
    ...
    invoke(*conn, msg.method(), msg.data(),
           [conn_weak, id](rpc_json_result result)
           {
             auto conn_ptr = conn_weak.lock();
             if (!conn_ptr) return;
             if (result.success())
               conn_ptr->send_message(
                   response_message(id, std::move(result._data)));
             else
               conn_ptr->send_message(error_message(
                   id, result._error_code, std::move(result._error_text),
