  ...
  while (true)
  {
    if (buffer_off == buffer.size())
    {
      ...
      else
      {
        ...
        memcpy(&buffer[0], &buffer[buffer_start_off],
    ...
    ssize_t n =
        read_data(&buffer.data()[buffer_off], buffer.size() - buffer_off);
    if (n <= 0) return;
    ...
    {
      ssize_t c = current_encoding.load()->check_read_message_complete(
          &buffer.data()[buffer_start_off], buffer_off - buffer_start_off,
          check_start - buffer_start_off);
      if (c < 0) break;
      bool has_error = true;
      try
      {
        current_encoding.load()->read_message(&buffer.data()[buffer_start_off],
                                              (size_t)c, current_message);
        has_error = false;
      }
      ...
      {
        on_error(rpc_call_exception(rpc_error_codes::parse_error,
                                    rpc_error_codes::to_string));
      }
      catch (nlohmann::json::exception& e)
      {
        on_error(rpc_call_exception(rpc_error_codes::invalid_request,
                                    rpc_error_codes::to_string));
      }
      catch (std::exception& e)
      {
        on_error(rpc_call_exception(rpc_error_codes::internal_error,
                                    rpc_error_codes::to_string));
      }
      if (!has_error) on_message(current_message);
