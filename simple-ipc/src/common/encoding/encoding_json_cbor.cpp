...
void encoding::json_cbor::send_message(connection_internal& conn,
...
void encoding::json_cbor::send_json(connection_internal& conn,
...
ssize_t encoding::json_cbor::check_read_message_complete(const char* buf,
                                                         size_t buf_size,
                                                         size_t last_read_off)
{
  ...
  if (buf_size >= total_size) return total_size;
  return -1;
}

void encoding::json_cbor::read_message(const char* buf, size_t buf_size,
                                       message_container& ret)
{
  size_t varint_size = buf_size;
  auto size = varint::decode_unsigned(buf, buf_size, &varint_size);
  nlohmann::json data =
      nlohmann::json::from_cbor(nlohmann::detail::input_adapter(
          &buf[varint_size], buf_size - varint_size));
