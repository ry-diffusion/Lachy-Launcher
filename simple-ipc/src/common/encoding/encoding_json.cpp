...
void encoding::json::send_message(connection_internal& conn,
                                  rpc_message const& msg)
{
  nlohmann::json data;
  if (msg.has_id()) data["id"] = msg.id();
...
void encoding::json::send_json(connection_internal& conn,
...
ssize_t encoding::json::check_read_message_complete(const char* buf,
                                                    size_t buf_size,
...
void encoding::json::read_message(const char* buf, size_t buf_size,
                                  message_container& ret)
{
  nlohmann::json data =
      nlohmann::json::parse(nlohmann::detail::input_adapter(buf, buf_size));
  ...
  else if (data.count("error") > 0)
  {
    ...
    if (msg_id != data.end() && msg_id->is_number())
      ret.set(error_message(*msg_id, error_data["code"], error_data["message"],
                            error_data["data"]));
    else
      ret.set(error_message(error_data["code"], error_data["message"],
                            error_data["data"]));
  }
  ...
  {
    throw rpc_call_exception(simpleipc::rpc_error_codes::invalid_request,
                             simpleipc::rpc_error_codes::to_string);
  }
}