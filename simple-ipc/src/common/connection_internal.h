...
namespace simpleipc
{
  ...
  {
   public:
    using message_callback = std::function<void(std::string const& method,
   ...
   public:
    connection_internal()
        : buffer(BUFFER_SIZE),
          current_encoding(encoding::encodings::get_default_encoding())
    {
    }
   ...
   protected:
    ...
    void on_error(rpc_call_exception_interface const& ex_as_rpc,
