...
{
  fd = epoll_create1(0);
  if (fd < 0) throw std::runtime_error("Failed to create epoll fd");
...
void epoll_io_handler::add_socket(int fd, fd_callback data_cb,
                                  fd_callback close_cb)
{
...
void epoll_io_handler::run()
{
  ...
  while (true)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      if (cb == cbs.end()) continue;
      if ((events[i].events & EPOLLRDHUP) || (events[i].events & EPOLLHUP))
      {
        if (cb->second.close_cb) cb->second.close_cb(events[i].data.fd);
      }
      else if (events[i].events & EPOLLIN)
      {
        if (cb->second.data_cb) cb->second.data_cb(events[i].data.fd);
      }
    }
    if (!running)
    {
