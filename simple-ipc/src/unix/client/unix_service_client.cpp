...
void unix_service_client_impl::open(std::string const& path)
{
  ...
  if (fd < 0)
    throw std::runtime_error("Failed to open socket: socket() returned " +
                             std::to_string(fd));
  sockaddr_un addr;
  ...
  if (cb) cb.load()->connection_opened();
}
...
std::unique_ptr<service_client_impl>
service_client_impl_factory::create_platform_service()
{
