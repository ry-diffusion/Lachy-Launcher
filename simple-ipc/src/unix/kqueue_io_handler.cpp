...
void kqueue_io_handler::add_socket(int fd, fd_callback data_cb,
...
void kqueue_io_handler::run()
{
  ...
  while (true)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      {
        if (cb->second.close_cb) cb->second.close_cb(ev_list[i].ident);
      }
      else if (ev_list[i].filter & EVFILT_READ)
      {
        if (cb->second.data_cb) cb->second.data_cb(ev_list[i].ident);
      }
    }
