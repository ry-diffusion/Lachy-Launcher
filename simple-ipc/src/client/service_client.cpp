...
void service_client::connection_closed()
{
  ...
  cb_mutex.unlock();
  for (auto& cb : cbs)
    cb.second(rpc_json_result::error(rpc_error_codes::connection_closed,
...
void service_client::send_message(rpc_message const& msg)
{
  ...
  if (!hello_response_value && msg.method() != ".hello")
  {
    ...
    if (cb)
      cb(rpc_json_result::error(simpleipc::rpc_error_codes::no_hello_reply,
                                "Haven't got hello reply yet"));
    return;
  }
  lock.unlock();
...
{
  if (!msg.has_id()) return;
  auto cb = get_rpc_cb(msg.id());
  if (cb) cb(rpc_json_result::response(msg.data()));
}
      ...
      .call(
          [this](rpc_json_result res)
          {
            if (res.success())
            {
              ...
              if (conn != nullptr)
                ((connection_internal*)conn)
                    ->set_encoding(encoding::encodings::get_encoding_by_name(
                        res.data().at("encoding")));
            }
...
{
  std::unique_lock<std::mutex> lock(hello_response_mutex);
  hello_response_cv.wait(lock,
                         [this]()
                         {
                           return hello_response_value;
                         });
}