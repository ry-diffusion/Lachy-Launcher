  ...
  namespace client
  {
    ...
    class rpc_call<void>
    {
     private:
      ...
      {
        if (r.success()) return rpc_result<void>::response();
        return rpc_result<void>::error(r.error_code(), r.error_text(),
                                       r.error_data());
      }
    ...
    class rpc_call
    {
     private:
      ...
      static rpc_result<T> convert(rpc_json_result const& r,
                                   rpc_call_conversion_func<T> cf)
      {
        if (r.success())
        {
          ...
          {
            return rpc_result<T>::error(rpc_error_codes::internal_error,
                                        e.what());
          }
        }
        return rpc_result<T>::error(r.error_code(), r.error_text(),
                                    r.error_data());
      }

     public:
      rpc_call(rpc_json_call c, rpc_call_conversion_func<T> cf)
          : c(std::move(c)), cf(cf)
      {
      }
