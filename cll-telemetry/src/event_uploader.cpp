...
EventUploadStatus EventUploader::sendEvents(BatchedEventList& batch,
                                            bool compress, bool canRetry)
{
  ...
  {
    compressed =
        EventCompressor::compress(batch.getData(), batch.getDataSize());
  ...
  for (auto const& h : steps) h->onRequest(userReq);
  for (auto const& h : userReq.headers) req->addHeader(h.first, h.second);
  ...
  if (res.status == 200) return EventUploadStatus::success();

  if (res.status == 429 || res.status == 503)
  {
    auto retryAfter = res.findHeader("Retry-After");
    if (retryAfter.empty()) return EventUploadStatus::error();
    return EventUploadStatus::rateLimit(
        std::chrono::seconds(std::stoull(retryAfter)));
  }
  if (res.status == 401)
  {
    bool retry = false;
    for (auto const& h : steps) retry |= h->onAuthenticationFailed();
    if (retry && canRetry) return sendEvents(batch, compress, false);
  }
  return EventUploadStatus::error();
}

std::string EventUploader::getUploadTimestamp(
    std::chrono::system_clock::time_point time)
{
  ...
  snprintf(&timestamp[timestampLen], sizeof(timestamp) - timestampLen, ".%06dZ",
           (int)timeMs.count());
  return std::string(timestamp);
}

EventUploadStatus EventUploader::sendEvents(EventBatch& batch, size_t maxCount,
                                            size_t maxSize)
{
  ...
  if (!events)
  {
    if (batch.hasEvents())
    {
      Log::warn("EventUploader",
                "Failed to get any event from the batch -"
                "dropping the first event and trying again");
      auto ev = batch.getEventsForUpload(1, SIZE_MAX);
      if (!ev) return EventUploadStatus::error();
  ...
  if (ret) batch.onEventsUploaded(*events);
  return ret;
}