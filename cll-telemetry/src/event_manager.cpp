...
EventManager::EventManager(std::string const& iKey,
                           std::string const& batchesDir,
                           std::string const& cacheDir)
    : iKey(iKey),
      httpClient(http::HttpClient::createPlatformClient()),
      uploader(*httpClient)
{
  config.setCache(std::unique_ptr<ConfigurationCache>(
      new FileConfigurationCache(cacheDir + "/config_cache.json")));
  config.addDefaultConfigurations(iKey);
  config.addUpdateCallback(
      std::bind(&EventManager::onConfigurationUpdated, this));
  ...
  normalStorageBatch = std::unique_ptr<EventBatch>(
      new BufferedEventBatch(std::move(normalStorageBatch), 50));
  ...
  realtimeUploadTask =
      std::unique_ptr<TaskWithDelayThread>(new TaskWithDelayThread(
...
{
  ((MultiFileEventBatch&)*((BufferedEventBatch&)*normalStorageBatch)
       .getWrapped())
      .setFileLimits((size_t)config.getMaxEventsPerPost(),
                     (size_t)config.getMaxEventSizeInBytes());
  ((MultiFileEventBatch&)*criticalStorageBatch)
      .setFileLimits((size_t)config.getMaxEventsPerPost(),
                     (size_t)config.getMaxEventSizeInBytes());
  uploaderMaxSize.store((size_t)config.getMaxEventSizeInBytes());
  uploaderMaxEvents.store((size_t)config.getMaxEventsPerPost());
...
void EventManager::uploadTasks()
{
  ...
  updateConfigIfNeeded();
  while (!mainUploadTask->isStopping() &&
         (normalStorageBatch->hasEvents() || criticalStorageBatch->hasEvents()))
  {
    updateConfigIfNeeded();
    EventUploadStatus status;
    if (criticalStorageBatch->hasEvents())
      status = uploader.sendEvents(*criticalStorageBatch, uploaderMaxEvents,
                                   uploaderMaxSize);
    else if (normalStorageBatch->hasEvents())
      status = uploader.sendEvents(*normalStorageBatch, uploaderMaxEvents,
                                   uploaderMaxSize);
    else
      break;
...
void EventManager::uploadRealtimeTasks()
{
  ...
  {
    updateConfigIfNeeded();
    if (!uploader.sendEvents(realtimeMemoryBatch, uploaderMaxEvents,
                             uploaderMaxSize))
    {
      // if the upload fails, transfer the events to the disk-backed queues, and
      // queue an immediate reupload this lets us handle the backoff only in
      // that function
