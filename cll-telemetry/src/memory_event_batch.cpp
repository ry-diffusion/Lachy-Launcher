...
std::unique_ptr<BatchedEventList> MemoryEventBatch::getEventsForUpload(
    size_t maxCount, size_t maxSize)
{
  std::lock_guard<std::mutex> lock(mutex);
  if (maxCount == 0 || maxSize == 0 || items.empty()) return nullptr;
  std::vector<char> ret;
  size_t s = 0;
  size_t count = 0;
  ...
  return std::unique_ptr<BatchedEventList>(
      new VectorBatchedEventList(ret, count, count < items.size()));
}
...
void MemoryEventBatch::onEventsUploaded(BatchedEventList& events)
{
  ...
  items.erase(
      items.begin(),
      items.begin() + ((VectorBatchedEventList&)events).getEventCount());
}
