...
void CurlHttpRequest::setPlatformCurlInitHook(
    std::function<void(CURL* curl)> func)
{
...
CurlHttpRequest::CurlHttpRequest()
{
  ...
  if (platformCurlInitHook) platformCurlInitHook(curl);
}
...
void CurlHttpRequest::addHeader(std::string const& name,
...
size_t CurlHttpRequest::curlHeaderHandler(char* buffer, size_t size,
                                          size_t nitems, HttpResponse* data)
{
  ...
  if (ptr != nullptr)
  {
    ...
    data->headers.emplace_back(
        std::string(buffer, iof),
        std::string(&buffer[iof + 2], size * nitems - iof - 2 - 2));
  }
  return nitems * size;
}

size_t CurlHttpRequest::curlOutputHandler(void* ptr, size_t size, size_t nmemb,
                                          std::ostream* s)
{
...
int CurlHttpRequest::curlDebugCallback(CURL* handle, curl_infotype type,
...
HttpResponse CurlHttpRequest::send()
{
  ...
  if (ret != CURLE_OK) throw CurlError(ret);
