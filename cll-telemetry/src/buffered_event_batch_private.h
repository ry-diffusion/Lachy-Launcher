...
namespace cll
{
  ...
  class WrapperBufferedEventList : public BufferedEventList
  {
   public:
    ...
    WrapperBufferedEventList(Type type,
                             std::unique_ptr<BatchedEventList> wrapped,
                             bool hasMemoryEvents = false)
        : BufferedEventList(type),
          wrapped(std::move(wrapped)),
          hasMemoryEvents(hasMemoryEvents)
    {
    }
  ...
  class MergedBufferedEventList : public BufferedEventList
  {
   public:
    ...
    MergedBufferedEventList(std::unique_ptr<BatchedEventList> wrapped,
                            std::unique_ptr<BatchedEventList> mem)
        : BufferedEventList(Type::Merged),
          wrapped(std::move(wrapped)),
          mem(std::move(mem))
    {
