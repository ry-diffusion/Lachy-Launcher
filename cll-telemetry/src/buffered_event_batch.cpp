...
std::unique_ptr<BatchedEventList> BufferedEventBatch::getEventsForUpload(
    size_t maxCount, size_t maxSize)
{
  ...
  if (wrapped->hasEvents())
  {
    ...
    {
      auto retMem = mem.getEventsForUpload(maxCount - ret->getEventCount(),
                                           maxSize - ret->getDataSize());
      if (retMem)
        return std::unique_ptr<BatchedEventList>(
            new MergedBufferedEventList(std::move(ret), std::move(retMem)));
    }
    if (ret != nullptr)
      return std::unique_ptr<BatchedEventList>(new WrapperBufferedEventList(
          BufferedEventList::Type::Wrapped, std::move(ret), mem.hasEvents()));
  }
  auto ret = mem.getEventsForUpload(maxCount, maxSize);
  return std::unique_ptr<BatchedEventList>(new WrapperBufferedEventList(
      BufferedEventList::Type::Memory, std::move(ret)));
}
...
void BufferedEventBatch::flushInternal()
{
  ...
  for (auto const& item : items)
  {
    if (!wrapped->addEvent(item))
      Log::warn("BufferedEventBatch",
                "Failed to forward an event to the underlying implementation");
  }
}
