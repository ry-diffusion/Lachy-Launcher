...
EventSerializer::EventSerializer()
{
  ...
  std::uniform_int_distribution<long long> ldist(
...
std::string EventSerializer::getEventTimeAsString(
    std::chrono::system_clock::time_point timepoint)
{
  ...
  auto timeMs = duration_cast<milliseconds>(timepoint.time_since_epoch()) -
                duration_cast<milliseconds>(
                    duration_cast<seconds>(timepoint.time_since_epoch()));
  snprintf(&timestamp[timestampLen], sizeof(timestamp) - timestampLen, ".%03dZ",
           (int)timeMs.count());
...
nlohmann::json EventSerializer::createEnvelopeFor(Event const& ev)
{
  ...
  if (!os.empty()) envelope["os"] = os;
  if (!osVer.empty()) envelope["osVer"] = osVer;
  if (!appId.empty()) envelope["appId"] = appId;
  if (!appVer.empty()) envelope["appVer"] = appVer;
  if (!extensions.empty())
  {
    auto& ext = envelope["ext"] = nlohmann::json::object();
    for (auto const& e : extensions) ext[e->getName()] = e->build(ev);
  }
  return envelope;
}