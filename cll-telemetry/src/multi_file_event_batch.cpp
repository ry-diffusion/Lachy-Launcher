...
MultiFileEventBatch::MultiFileEventBatch(std::string path, std::string prefix,
                                         std::string suffix,
  ...
  while ((dp = readdir(dir)) != nullptr)
  {
    if (dp->d_type == DT_REG)
    {
      ...
      if (len < prefix.length() + suffix.length() ||
          ...
          memcmp(&dp->d_name[len - suffix.length()], suffix.data(),
                 suffix.length()) != 0)
        continue;
      std::string e(&dp->d_name[prefix.length()],
...
std::unique_ptr<BatchedEventList> MultiFileEventBatch::getEventsForUpload(
    size_t maxCount, size_t maxSize)
{
  std::lock_guard<std::mutex> l(batchPointerMutex);
  checkOldestBatch();
  if (oldestBatch)
  {
    std::unique_ptr<BatchedEventList> ret =
        oldestBatch->getEventsForUpload(maxCount, maxSize);
    if (!ret) return nullptr;
    return std::unique_ptr<BatchedEventList>(
        new EventList(std::move(ret), true));
  }
  else
  {
    // the newest batch is not set as the oldest batch, so we need to check for
    // it here
    std::unique_ptr<BatchedEventList> ret =
        newestBatch->getEventsForUpload(maxCount, maxSize);
    if (!ret) return nullptr;
    return std::unique_ptr<BatchedEventList>(
        new EventList(std::move(ret), false));
  }
}
...
void MultiFileEventBatch::onEventsUploaded(BatchedEventList& events)
{
  ...
  if (oldestBatch)
  {
    ...
    // if the batch has no more events, reset the pointer; the oldestBatch
    // pointer will be auto recreated as needed when checkOldestBatch() is
    // called the batch file will be automatically deleted by the
    // onEventsUploaded if there are no more events
    if (!oldestBatch->hasEvents())
    {
      oldestBatch.reset();
    }
...
{
  std::lock_guard<std::mutex> l(batchPointerMutex);
  return !oldBatches.empty() || oldestBatch != nullptr ||
         newestBatch->hasEvents();
}
...
void MultiFileEventBatch::setFileLimits(size_t maxEvents, size_t maxSize)
{
  ...
  if (oldestBatch) oldestBatch->setLimit(maxEvents, maxSize);
