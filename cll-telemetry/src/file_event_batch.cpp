...
FileEventBatch::FileEventBatch(std::string const& path) : path(path)
{
  ...
  if (fd < 0)
    Log::warn("FileEventBatch",
              "Error occurred trying to open the specified file");
...
void FileEventBatch::countEventsInFile()
{
  ...
  {
    Log::warn("FileEventBatch",
              "Invalid event at the end of the file; trimming the file from "
              "%li to %lu",
...
bool FileEventBatch::addEvent(std::string const& data)
{
  ...
  {
    Log::warn("FileEventBatch",
              "Trying to add an event to a finalized EventBatch");
    return false;
  }
  if (!canAddEventInt(data.length())) return false;
  if (!streamAtEnd) seekToEndAndGetFileSize();
  size_t o = 0;
  while (o < data.size())
  {
    ...
    {
      Log::warn("FileEventBatch",
                "Failed to write an event due to an IO error");
      if (o > 0)
      {
        // We got into a corrupted state pretty much. Try to remove the event
        // using truncate.
...
std::unique_ptr<BatchedEventList> FileEventBatch::getEventsForUpload(
    size_t maxCount, size_t maxSize)
{
  std::lock_guard<std::mutex> lock(streamMutex);
  if (fd < 0 || fileSize == 0 || maxCount == 0) return nullptr;
  lseek(fd, 0, SEEK_SET);
  streamAtEnd = false;
  ...
  size_t events = 0;
  ssize_t m;
  while ((m = read(fd, &data.data()[n], maxSize - n)) > 0) n += m;
  auto data_ptr = data.data();
  char* end_data_ptr = data.data() + n;
  while (true)
  {
    char* n_data_ptr = (char*)memchr(data_ptr, '\n', end_data_ptr - data_ptr);
    if (n_data_ptr == nullptr) break;
    n_data_ptr++;

    data_ptr = n_data_ptr;
    if (++events == maxCount) break;
  }
  n = data_ptr - data.data();
  data.resize(n);
  return std::unique_ptr<BatchedEventList>(
      new VectorBatchedEventList(std::move(data), events, n < fileSize));
}
...
void FileEventBatch::onEventsUploaded(BatchedEventList& events)
{
  ...
  if (fdIn < 0)
    throw std::runtime_error(
        "Failed to open input stream for onEventsUploaded");
  ...
  while ((m = read(fdIn, buf, bufSize)) > 0)
  {
    for (size_t o = 0; o < m;)
    {
      ...
      {
        Log::warn("FileEventBatch",
                  "onEventsUploaded: failed to write data from buffer");
        break;
      }
      o += m2;
      n += m2;
