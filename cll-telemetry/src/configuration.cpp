...
{
  template <>
  void ConfigurationProperty<int>::set(nlohmann::json const& json,
...
bool Configuration::download(HttpClient& client, ConfigurationCache* cache)
{
  ...
  if (requestStart < downloadMinRetryTime) return false;

  Log::trace("Configuration", "Downloading configuration from: %s",
             url.c_str());
  ...
  if (cache) hasCached = cache->readFromCache(url, cached);
  ...
  if (hasCached) request->addHeader("If-None-Match", "\"" + cached.etag + "\"");
  ...
  {
    Log::warn("Configuration", "Failed to download configuration: %s",
              error.what());
  }
  ...
  {  // ok
    ...
    cached.refreshInterval =
        std::chrono::minutes(JsonUtils::asInt(val["refreshInterval"]));
    ...
    downloaded = true;
    if (cache) cache->writeConfigToCache(url, cached);
  ...
  Log::warn("Configuration",
            "Failed to download configuration: status code %li",
            response.status);
  ++downloadAttempt;
  // pick a time between <2 seconds, ~5 minutes>
  downloadMinRetryTime =
      requestStart +
      std::chrono::seconds(1 << std::min(downloadAttempt + 1, 8));
  return false;
}
