...
namespace cll
{
  ...
  class EventManager
  {
   ...
   public:
    /**
     * Creates the Event Manager with the specified application-specific
     * instrumentation key.
     ...
     */
    EventManager(std::string const& iKey, std::string const& batchesDir,
     ...
     */
    inline std::string const& getIKey() const
    {
      return iKey;
    }

    /***
     * Adds an EventUploader step, which allows you to add custom authentication
    ...
    /**
     * Starts the event upload threads as needed. No setters should be called
