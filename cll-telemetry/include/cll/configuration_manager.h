...
namespace cll
{
  ...
  class ConfigurationManager
  {
   ...
   public:
    ...
    {
      // NOTE: the current Android client also sends extra params that include
      // the OS name and version as well as device id. It doesn't seem that they
      // affect anything, so I am skipping that part.
      ...
      configurations.push_back(std::unique_ptr<Configuration>(new Configuration(
          "https://settings.data.microsoft.com/settings/v2.0/telemetry/" +
          iKey)));
    }
    ...
    template <typename T, ConfigurationProperty<T> Configuration::*Property,
              T def>
    T get()
    {
      for (auto const& c : configurations)
      {
        if ((c.get()->*Property).isSet()) return (c.get()->*Property).get();
      }
      return def;
    }
    ...
    bool hasDownloadedConfigs()
    {
      ...
      {
        if (!config->downloaded) return false;
      }
      return true;
    }
    ...
    void downloadConfigs(http::HttpClient& client)
    {
      ...
      for (auto const& config : configurations)
      {
        ...
        {
          if (config->download(client, cache.get())) downloadedAnything = true;
        }
      }
      if (downloadedAnything)
      {
        for (auto const& c : updateCallbacks) c();
      }
    }
