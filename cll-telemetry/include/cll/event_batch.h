...
namespace cll
{
  ...
  class EventBatch
  {
   public:
    ...
    virtual std::unique_ptr<BatchedEventList> getEventsForUpload(
        size_t maxCount, size_t maxSize) = 0;
  ...
  class VectorBatchedEventList : public BatchedEventList
  {
   ...
   public:
    explicit VectorBatchedEventList(std::vector<char> data, size_t events,
                                    bool hasMoreEvents)
