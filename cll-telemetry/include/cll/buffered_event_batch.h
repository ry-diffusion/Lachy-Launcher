...
namespace cll
{
  ...
  {
   private:
    // I added the mutex, because a edge case in flush() is possible where the
    // list would be transferred and for a while nothing would have events
   ...
   public:
    BufferedEventBatch(std::unique_ptr<EventBatch> wrapped,
                       size_t bufferedItemCount)
