...
namespace cll
{
  ...
  class EventUploader
  {
   ...
   public:
    EventUploader(
        http::HttpClient& client,
        std::string url = "https://vortex.data.microsoft.com/collect/v1")
        : client(client), url(std::move(url))
    {
    }

    EventUploadStatus sendEvents(BatchedEventList& batch, bool compress = false,
                                 bool canRetry = true);

    EventUploadStatus sendEvents(EventBatch& batch, size_t maxCount,
                                 size_t maxSize);
