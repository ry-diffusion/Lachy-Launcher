...
static void CleanUpBatch(MultiFileEventBatch& batch)
{
  ...
  if (dir != nullptr)
  {
    ...
    {
      if (dp->d_type == DT_REG) remove((batch.getPath() + dp->d_name).c_str());
    }
    closedir(dir);
  }
...
TEST_F(MultiFileEventBatchTest, BasicTest)
{
  ...
  batch = std::unique_ptr<MultiFileEventBatch>(
      new MultiFileEventBatch("multifile_test", "events", ".txt", 2, 1024));
...
TEST_F(MultiFileEventBatchTest, ReadIncrementalWithRemoval)
{
  mkdir("multifile_test", 0700);
  batch = std::unique_ptr<MultiFileEventBatch>(
      new MultiFileEventBatch("multifile_test", "events", ".txt", 10, 320));
