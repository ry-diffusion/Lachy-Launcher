...
namespace
{
  ...
  TEST_F(FileEventBatchTest, ItemCountTest)
  {
    ...
    for (int i = 0; i < 32; i++) batch.addEvent(event);
  ...
  TEST_F(FileEventBatchTest, CountLimitTest)
  {
    ...
    for (int i = 0; i < 2; i++) ASSERT_TRUE(batch.addEvent(event));
  ...
  TEST(FileEventBatchCustomTest, PersistenceTest)
  {
    ...
    }
    {
      ...
      auto upEv = EventBatchTest::GetMessagesInEventList(
          batch.getEventsForUpload(10, 512).get());
  ...
  TEST_F(FileEventBatchTest, FinalizedDeletion)
  {
    ...
    // Make sure the file no longer exists (it was finalized, and all events
    // should be removed)
