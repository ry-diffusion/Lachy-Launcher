...
void EventBatchTest::BasicTest(EventBatch& batch)
{
  ...
  // Get the event back
  auto upEv =
      batch.getEventsForUpload(1, 128);  // this should return event + "\r\n"
...
void EventBatchTest::ReadIncrementalWithRemoval(cll::EventBatch& batch,
                                                size_t eventCount)
{
  size_t maxCount = 1;
  size_t gotEvents = 0;
  while (gotEvents < eventCount)
  {
    ...
    ASSERT_GT(val->getDataSize(), 0)
        << "Got count: " << gotEvents << "; Max count: " << maxCount;
...
std::vector<std::string> EventBatchTest::GetMessagesInEventList(
    cll::BatchedEventList* val)
{
  char const* ptr = val->getData();
  std::vector<std::string> list;
  if (val == nullptr) return list;
  while (true)
  {
    char const* e = (char const*)memchr(
        ptr, '\n', val->getDataSize() - (ptr - val->getData()));
    if (e == nullptr)
    {
      if (ptr != val->getData() + val->getDataSize())
        throw std::runtime_error(
            "getMessagesInEventList: Has extra data after message");
      break;
    }
    if (*e != '\n' || *(e - 1) != '\r')
      throw std::runtime_error(
          "getMessagesInEventList: Doesn't end with \\r\\n");
